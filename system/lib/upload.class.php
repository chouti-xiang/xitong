...
class Upload{
	var $saveName;// 保存名
	var $savePath ;// 保存路径
	var $fileFormat = array('gif','JPG','jpg','jpeg','doc','application/octet-stream');// 文件格式&MIME限定
...
// @param $fileArray 文件信息数组
	function copyfile($fileArray){
		...
		if(!move_uploaded_file($fileArray["tmp_name"], $this->savePath.$this->saveName)){
			...
			$this->returninfo['path'] =  $this->savePath.$this->saveName;
			$this->returninfo['thumbpath'] =  $this->savePath.'_thumb_'.$this->saveName;
			/*
...
// 文件格式检查,MIME检测
	function validateFormat(){
		...
		//d(in_array(strtolower($this->ext), $this->fileFormat));
		//d(in_array(strtolower($this->returninfo['type']), $this->fileFormat));
...
// 创建多级目录
// @param $savePath 这个目录目前暂时支持在根目录之下生成的目录 返回的参数是按照日期进行的多级目录
	function setMkdir($savePath){
		$time = date("y/m/d");
		$dir = APP_ROOT."/".$savePath."/".$time;
		$this->savePath = $savePath."/".$time."/";
		if(file_exists($dir)){
